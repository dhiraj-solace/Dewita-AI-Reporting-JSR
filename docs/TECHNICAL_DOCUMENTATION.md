...
- Validator performance defaults: `LLM_VALIDATOR_ENABLED=false`, `LLM_VALIDATOR_TIMEOUT_SECONDS=20`, `LLM_VALIDATOR_MAX_RETRIES=3`
...
- Intent safety: `OPENROUTER_API_KEY`, optional `OPENROUTER_INTENT_MODEL`
...
- Calls OpenRouter-backed `validate_user_query_safety()` before generation.
...
OpenRouter-backed user intent safety checker.

Important function:

- `validate_user_query_safety(query)`: Classifies the natural-language request by intent and allows only read-only reporting intent.

This avoids false positives from words like `created`, `updated`, or `deleted` when they describe report data, while still blocking requests that intend to mutate data, change schema, perform admin actions, request credentials, or execute code.
...
- OpenRouter intent safety classification
...
3. Backend validates the user question with `validate_user_query_safety()`.
   - Uses OpenRouter to classify intent instead of rejecting by keywords.
   - Allows read/report/list/count/summary-style intent.
   - Blocks create/update/delete/admin/schema/credential/code intent.
...
`query_safety.py` uses OpenRouter to classify the user's intent before SQL generation. It no longer blocks solely because a keyword appears in the text. This allows safe report questions such as "show last month deleted users" or "list created users" when the intent is to view historical records.

Allowed intent examples include read reports, list records, count records, summarize records, compare records, and export reports.

Blocked intent examples include create record, update record, delete record, schema change, admin action, credential request, and execute code.

If OpenRouter intent classification is unavailable, the request is blocked safely instead of falling through to SQL generation.
